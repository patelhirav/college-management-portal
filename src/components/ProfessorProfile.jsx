...
const AdminProfile = () => {
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      try {
        const data = await apiService.getSubAdminProfile();
        setProfileData(data);
        if (data.bio) {
  ...
  const handleSave = async () => {
    try {
      if (profileData.bio) {
        await apiService.updateSubAdminBio(bioForm); // update
      } else {
        await apiService.addSubAdminBio(bioForm); // add
      }
      const updatedProfile = await apiService.getSubAdminProfile(); // refresh
  ...
  return (
    <div className="admin-profile">
      <div className="profile-header">
        ...
        <div className="profile-id">Professor</div>
      </div>
